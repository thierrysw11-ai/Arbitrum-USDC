 ...
 * about a wallet's Aave V3 Arbitrum position. The tools wrap our existing
 * server-side reader (`server.ts`) and pure-math layer (`math.ts`), so
 * the numbers an agent quotes are exactly the numbers shown in the
 * Portfolio UI — no parallel implementation drift.
 *
 * Each entry exports:
...
// --------------------------------------------------------------------------
// Recent activity — subgraph query (server-side fetch, no proxy hassle)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Swissborg (Ethereum mainnet) — ERC-20 transfer indexer
//
// Schema (per the deployed subgraph):
//   transfer { id, from, to, value, timestamp, txHash, blockNumber, block }
//   approval { id, owner, spender, value, timestamp, txHash, blockNumber, block }
//
// We derive the gateway URL from the existing NEXT_PUBLIC_GRAPH_API_KEY so
// the user doesn't have to set a second env var. The fetch runs server-side
// from the agent route, so the key never reaches the browser through this
// path even though the var is publicly named.
// --------------------------------------------------------------------------
...
async function fetchSwissborgActivity(
  ...
  // The Graph supports nested `where` with multiple fields ANDed; for OR we
  // run two queries and merge. Cheap, and lets us tag direction at the
  // source.
  const query = `
  ...
  // BORG (and legacy CHSB) are 8-decimal ERC-20s. We expose both the raw
  // wei-style value and a human-readable float. If the indexed token turns
  // out to use different decimals the caller can recompute from `value`.
  const DECIMALS = 8n;
  ...
  const formatRaw = (v: string): number => {
    try {
      ...
      // Keep ~6 sig figs of fractional precision in the float.
  ...
  // Net balance from indexed history. Caveat: this is only correct if the
  // subgraph has indexed every transfer of this token from genesis; if it
  // started indexing late, the number is "balance change since indexing
  // start", not absolute balance. Surfaced as `note` so the agent can flag
  // the limitation.
  let net = 0n;
  for (const t of ins) net += BigInt(t.value);
  for (const t of out) net -= BigInt(t.value);

  // Merge + dedupe by id, sort desc by timestamp, keep top `limit`.
  const merged = [
...
// --------------------------------------------------------------------------
// On-chain BORG balance (Ethereum mainnet, ERC-20 balanceOf)
//
// The subgraph gives us indexed history. For an absolute, trust-the-chain
// balance we hit the token contract directly via viem. We also pull
// decimals + symbol on-chain so we don't have to hardcode them — handles
// the BORG/CHSB rebrand cleanly whichever contract the user configures.
// --------------------------------------------------------------------------

// Default to the legacy CHSB contract (the BORG token launched on this same
// address as a rebrand). Override with BORG_CONTRACT_ADDRESS in .env.local
// if the user holds a different deployment.
...
// One shared client per process. Public Llama RPC is fine for occasional
// reads; swap for Alchemy/Infura via env if rate limits bite.
...
async function fetchBorgBalance(
  ...
  // Single multicall: balanceOf + decimals + symbol. allowFailure off — we
  // want a hard error if any of the three reverts so the tool surfaces it
  // rather than silently lying about your balance.
